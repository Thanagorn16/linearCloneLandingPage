...
const makeNotifCard = (card, CardIcon) => {
	return (
		<div className={clsx(card.translateX, card.translateY, `flex flex-col p-4 items-start gap-2 w-120 h-30 translate-z-10 rounded-xl border border-[hsla(0,0%,100%,.17)] bg-[hsla(0,0%,16%,.5)] backdrop-blur-3xl
			transition-all  hover:bg-[hsla(0,0%,30%,.5)] hover:-translate-y-7
		`)}>
...
const  getCard = (card, icon, x, y, pos) => {
	return (
		<div className={clsx(pos, x, y, "flex items-center w-120 h-50 translate-z-10 rotate-y-20 -rotate-x-10 -rotate-z-3 mt-10")}>
			{makeNotifCard(card, icon)}
		</div>
	);
}
...
export default function NotifCard() {
	return (
		<div className="relative mt-10 w-95">
			{getCard(card1, redIcon, 'translate-x-0', '-translate-y-0')}
	        <div className="absolute w-145 h-70 inset-0 -translate-y-5  bg-gradient-to-l from-[#090a0b] to-transparent pointer-events-none"></div>
			{getCard(card2, orangeIcon, 'translate-x-10', '-translate-y-50', 'absolute')}
	        <div className="absolute w-145 h-70 inset-0 -translate-y-5  bg-gradient-to-l from-[#090a0b] to-transparent pointer-events-none"></div>
			{getCard(card3, greenIcon, 'translate-x-20', '-translate-y-40', 'absolute')}
	        <div className="absolute w-145 h-70 inset-0 -translate-y-5  bg-gradient-to-l from-[#090a0b] to-transparent pointer-events-none"></div>
		</div>
	);
}
